...
-- local tmp = io.open("debug_read", "w")
-- tmp:close();
local inFile = io.open("debug_read", "r")
local outFile = io.open("debug_write", "w")
-- outFile.setvbuf(outFile, 'line')
...
local function sendFully(str)
	outFile:write(str)
	-- outFile:close()
	-- outFile = io.open("debug_write", "w")
	-- outFile:flush() --S this crashes the binary, so i need creative workaround
	--S attemping to force a flush
	outFile:write("")
	outFile:write("")
	outFile:write("")
	outFile:write("")
...
local function logToDebugConsole(output, category)
	print(output) -- this one is mike's
end
...
local function recvMessage()
	...
	--S if (header == nil) then
		-- 디버거가 떨어진 상황 when the debugger is down
		--S return nil
	--S end
	...
	local body = nil
	while body == nil do
		body = inFile:read(bodySize)
	end

	if dumpCommunication then
		logToDebugConsole('[RECEIVED] ' .. body)
	end
